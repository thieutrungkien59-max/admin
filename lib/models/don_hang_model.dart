...
class DonHangModel {
  ...
  factory DonHangModel.fromJson(Map<String, dynamic> json) {
    return DonHangModel(
      id: (json['maDh'] ?? json['id'] ?? '').toString(),
      maDon: (json['maDh'] ?? json['maDon'] ?? '#ORD-UNKNOWN').toString(),
      thoiGianCho: (json['thoiGianCho'] ?? json['ThoiGianCho'] ?? '00:00').toString(),
      trongLuong: _parseDouble(json['khoiLuong'] ?? json['trongLuong']),
      ...
      diemLayHang: (json['diaChiLay'] ?? json['diemLayHang'] ?? '').toString(),
      lienHeLayHang: (json['sdtNguoiNhan'] ?? json['lienHeLayHang'] ?? '').toString(),
      diemGiaoHang: (json['diaChiGiao'] ?? json['diemGiaoHang'] ?? '').toString(),
      lienHeGiaoHang: (json['sdtNguoiNhan'] ?? json['lienHeGiaoHang'] ?? '').toString(),
      duKienGiaoPhut: _parseInt(json['duKienGiaoPhut'] ?? json['DuKienGiaoPhut']),
      giaCuoc: (json['phiGiaoHang']?.toString() ?? json['giaCuoc'] ?? '0đ').toString(),
      hinhThucGiao: (json['hinhThucGiao'] ?? json['HinhThucGiao'] ?? 'Tiêu chuẩn').toString(),
      tienCod: (json['tienCod']?.toString() ?? json['TienCod'] ?? '0đ').toString(),
